...
public final class Flow {
  ...
  /**
   * Set the dispatcher, which should be prepared for an immediate call to {@link
   * Dispatcher#dispatch(Traversal, TraversalCallback)}, unless a Traversal is
   * already in progress. In that case, the call to the new dispatcher will be made only when
   * the outstanding {@link TraversalCallback#onTraversalCompleted()} call is made.
   */
  public void setDispatcher(Dispatcher dispatcher) {
    ...
    if (pendingTraversal.state == TraversalState.ENQUEUED) {
      // A traversal was enqueued while we were deaf, run it now.
   ...
   * This mechanism protects against out of order calls to this method and {@link #setDispatcher}
   * (e.g. if an outgoing activity is paused after an incoming one resumes).
   * <p>
   * If multiple traversals are enqueued, pending ones will not execute until a new dispatcher
   * is set.
   */
  ...
  public void resetTo(final Path path) {
    move(new PendingTraversal() {
      @Override protected void doExecute() {
        ...
        int count = 0;
        // Take care to leave the original screen instance on the stack, if we find it.  This
        // enables some arguably bad behavior on the part of clients, but it's still probably
  ...
  public void resetToUpStackOf(final Path path) {
    move(new PendingTraversal() {
      @Override protected void doExecute() {
