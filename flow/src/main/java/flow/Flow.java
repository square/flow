...
import flow.TraversalTransformer.Transform;
...
public final class Flow {
  ...
  private TraversalTransformer transformer = new EmptyTraversalTransformer();
  ...
  void setTransformer(@NonNull TraversalTransformer traversalTransformer) {
    this.transformer = traversalTransformer;
  }
  ...
  private abstract class PendingTraversal implements TraversalCallback {
    ...
    void dispatch(History nextHistory, Direction direction) {
      ...
      Traversal traversal = new Traversal(history, nextHistory, direction, keyManager);
      Transform transform = transformer.maybeTransform(traversal);
      if (transform != null) {
        traversal = new Traversal(history, transform.destination, transform.direction, keyManager);
      }
      dispatcher.dispatch(traversal, this);
    }
