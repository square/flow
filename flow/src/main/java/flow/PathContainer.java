 ...
 *
 * This takes care of saving the swapped out view state into its path, so that the view state can
...
public abstract class PathContainer {
  ...
  /**
   * Set on the container view to make path information available during a transition.
   */
  ...
  public final void executeTraversal(Flow.Traversal traversal,
    ...
    // See if we already have the direct child we want, and if so short circuit the transition.
    if (oldChild != null) {
